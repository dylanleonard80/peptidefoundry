...
interface GlassPeptideCardProps {
  card: PeptideCard;
  isMember: boolean;
...
function GlassPeptideCard({
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <CardContent className="p-3 flex flex-col flex-1 relative">
            ...
            {card.sizes && card.sizes.length > 0 && (
              <div className="flex gap-1 overflow-x-auto sm:flex-wrap sm:overflow-visible scrollbar-hide">
                {card.sizes.map((size, index) => {
                  const price =
                    card.prices?.[size] ??
                    (index === 0 ? card.startingPrice : undefined);
                  const displayPrice =
                    price && isMember
                      ? getMemberPrice(price, card.slug, size)
                      : price;
                  return (
                    <Badge
                      key={index}
                    ...
                    >
                      {displayPrice ? (
                        <span className="flex items-center gap-1">
                          {isMember && (
                            <Hexagon className="h-2.5 w-2.5 text-primary" />
                          )}
                          {size} - ${displayPrice}
                        </span>
                      ) : (
          ...
          <Button
            variant="ghost"
            size="sm"
            className="w-full mt-2 h-7 text-xs text-muted-foreground hover:text-primary hover:bg-primary/10 transition-all duration-300 pointer-events-none"
          >
            Learn more
  ...
  return (
    ...
    >
      {showGlowEffect ? (
        <div
          className={`${fullWidth ? "w-full" : "w-[200px] sm:w-[240px] flex-shrink-0"} h-[320px] relative rounded-lg`}
        >
          <GlowingEffect
            spread={40}
            glow={true}
...
export { GlassPeptideCard };
export type { GlassPeptideCardProps };
